	...
	public static function getEventsPref($input_parameters) {
        ...
        $query = VwEventPrefModel::query();
				...
				->where(function ($query) {
					$query->where('fk_events_group', '=', 0)
				    ->orWhereColumn('fk_events_group', 'id');
				});
				*/
		...
		*/
        ...
        foreach ($getQuery as $item) {
            ...
            $arrayToReturn[$count]['modified']									=   $item->modified->format('Y-m-d H:i:s');
            $arrayToReturn[$count]['inserted']									=   $item->inserted->format('Y-m-d H:i:s');
            ...
            $arrayToReturn[$count]['hypocenter']['name']                        =   $item->type_hypocenter__value;
            $arrayToReturn[$count]['hypocenter']['region']                      =   $item->region ?? '';
            $arrayToReturn[$count]['hypocenter']['inserted']                    =   $item->hyp_inserted->format('Y-m-d H:i:s');
            $arrayToReturn[$count]['hypocenter']['modified']                    =   $item->hyp_modified->format('Y-m-d H:i:s');
